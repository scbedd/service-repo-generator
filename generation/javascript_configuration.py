...
class JavascriptRepoConfiguration(BaseRepoConfiguration):
    def __init__(self, path_to_config):
        super().__init__(path_to_config)

    
    def generate_ci_configs(self):
        """ Used to populate a single packages-<moniker>.json

        Returns String
        """
        ci_config_source = os.path.join(self.root_dir, 'template', 'js_packages.json')
    ...
    def write_ci_configs(self):
        for moniker in self.monikers:
            ...
            package_json_dest = os.path.join(self.ci_config_folder, 'packages-{}.json'.format(moniker))
